...
namespace CatalogAPI.Tests.Services
{
    public class BibliotecaServiceTests
    {
        ...
        public BibliotecaServiceTests()
        {
            ...
            _service = new BibliotecaService(_repoMock.Object, _repoGameMock.Object, _infoToken, _publishEndpointMock.Object);
        }
        ...
        [Fact]
        public async Task BuyGameAsync_DevePublicarOrderPlacedEvent_QuandoJogoDisponivel()
        {
            // Arrange
            var gameId = 1;
            ...
            _publishEndpointMock.Setup(p => p.Publish(It.IsAny<OrderPlacedEvent>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
            ...
            _publishEndpointMock.Verify(p => p.Publish(It.IsAny<OrderPlacedEvent>(), It.IsAny<CancellationToken>()), Times.Once);
        }

        [Fact]
        public async Task BuyGameAsync_DevePublicarComDadosCorretos_QuandoJogoDisponivel()
        {
            // Arrange
            var gameId = 2;
            var jogo = new Jogo { Id = gameId, Titulo = "EA FC 26", Preco = 299.90m };

            OrderPlacedEvent? eventoCapturado = null;

            _repoGameMock.Setup(r => r.GetById(gameId)).ReturnsAsync(jogo);
            _repoMock.Setup(r => r.ExistsInLibrary(_infoToken.Id, gameId)).ReturnsAsync(false);
            _publishEndpointMock.Setup(p => p.Publish(It.IsAny<OrderPlacedEvent>(), It.IsAny<CancellationToken>()))
                .Callback<OrderPlacedEvent, CancellationToken>((ev, _) => eventoCapturado = ev)
            ...
            Assert.Equal(_infoToken.Id, eventoCapturado.UserId);
            Assert.Equal(gameId, eventoCapturado.JogoId);
            Assert.Equal(jogo.Titulo, eventoCapturado.Titulo);
            Assert.Equal(jogo.Preco, eventoCapturado.Preco);
        }
